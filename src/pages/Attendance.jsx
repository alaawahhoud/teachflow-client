  ...
  for (const u of urls) {
    try {
      ...
      const profiles = arr.map((p) => ({
        ...
        display_name: p.display_name || p.full_name || p.ar_name || p.name || p.username || null,
        class_id: Number(p.class_id ?? p.homeroom_class_id ?? p.main_class_id ?? p.assigned_class_id ?? p.classId ?? NaN) || null,
      }));
      const map = new Map();
      for (const pr of profiles) { if (pr.user_id) map.set(pr.user_id, pr); }
      return map;
    } catch {}
  }
  return new Map();
...
const normalizeRow = (r) => {
  ...
  return {
    ...
    initials: String(name).split(" ").filter(Boolean).map((n) => n[0]).join("").slice(0, 3).toUpperCase(),
  };
};
...
export default function Attendance() {
  ...
  const [selectedClass, setSelectedClass] = useState("All Classes"); // نخزن الـ ID كسترينغ
  ...
  const toggleStatusFilter = (status) => setStatusFilters((p) => ({ ...p, [status]: !p[status] }));
  ...
  const filteredTeachers = useMemo(() => {
    return teachers.filter((s) => {
      ...
      const matchClass =
        ...
        String(s._classId ?? "") === String(selectedClass);
    ...
    (async () => {
      try {
        ...
        const master = tea.map((t) => {
          ...
          return { id: Number(t.id), name: pr.display_name || t.name, classId: pr.class_id ?? null };
        });
  ...
  // تحميل حضور اليوم + دمج profile/classes
  useEffect(() => {
    let ignore = false;
    (async () => {
      setLoading(true);
    ...
    <div className="p-6 bg-[#F9FAFB] dark:bg-[#1F2937] min-h-screen text-gray-800 dark:text-white">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {loading ? (
              <tr><td className="py-6 px-4 text-sm text-gray-500 dark:text-gray-300" colSpan={6}>Loading…</td></tr>
            ) : err ? (
              <tr><td className="py-6 px-4 text-sm text-red-600" colSpan={6}>{err}</td></tr>
            ) : (
                  ...
                  <td>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setTeachers((prev) => prev.map((x) => (x.id === teacher.id ? { ...x, status: v } : x)));
                      }}
                    >
                    ...
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {!["Sick", "Death in family", "Transportation Delay"].includes(teacher.notes) && (
                        <input
                          ...
                          onChange={(e) => {
                            const v = e.target.value;
                            setTeachers((prev) => prev.map((x) => (x.id === teacher.id ? { ...x, notes: v } : x)));
                          }}
                        />
                      )}
                    </div>
      ...
      <div className="mt-6 flex flex-col sm:flex-row justify-end gap-4 relative" ref={exportRef}>
        <div className="relative">
          ...
          {showExportOptions && (
            <div className="absolute right-0 top-full mt-1 min-w-[160px] bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-xl shadow z-[9999]">
              <button onClick={handleExportPDF} className="w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700 text-sm">
                Export as PDF
              </button>
              <button onClick={handleExportExcel} className="w-full px-4 py-2 text-left hover:bg-gray-100 dark:hover:bg-gray-700 text-sm">
                Export as Excel
              </button>
            </div>
          )}
        ...
        <button onClick={handleSaveAttendance} className="px-4 py-2 bg-blue-600 text-white rounded-xl shadow hover:bg-blue-700">
          Save Attendance
        </button>
      </div>
    </div>
